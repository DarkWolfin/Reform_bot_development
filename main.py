...
async def check_active_users(message: types.Message):
    ...
    await bot.send_message(
        ...
        text="Здравствуйте, босс! Пришлите то, что хотите разослать!",
        parse_mode="html",
    )
    ...
    for user in range(len(users)):
        try:
            ...
            await bot.send_photo(
                chat_id=(users[user][0]), photo=photo_mailing, parse_mode="html"
            )
...
async def mailing_text(message: types.Message):
    ...
    for user in range(len(users)):
        try:
            await bot.send_message(
                chat_id=(users[user][0]), text=message.text, parse_mode="html"
            )
            await asyncio.sleep(0.1)
        except BotBlocked:
            cur_user_blocked.execute(
...
async def welcome(message: types.Message):
    ...
    Welcome_kb.add(
        InlineKeyboardButton("Приятно познакомиться!", callback_data="Welcome_btn0")
    )
    mess = (
        f"Здравствуйте 🖐, <b>{message.from_user.first_name}</b>! Рад, что вы заботитетсь о своем ментальном здоровье! "
    ...
    await bot.send_message(
        message.from_user.id, mess, parse_mode="html", reply_markup=Welcome_kb
    )
    await log_users(message)


@dp.callback_query_handler(
    lambda c: c.data and c.data.startswith("Welcome_btn"),
    state=FSM_classes.MultiDialog.menu,
)
...
async def main_menu(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(
        message.from_user.id,
        "Вы в главном меню! Не знаете, что делать дальше?"
...
async def contacts(message: types.Message):
    await bot.send_message(
        ...
        parse_mode="html",
        reply_markup=Markups.cont,
    )
    await log_users(message)


@dp.callback_query_handler(
    lambda c: c.data and c.data.startswith("fullversion"), state=FSM_classes.MultiDialog
)
async def fullversion_callback(callback_query: types.CallbackQuery, state: FSMContext):
    await bot.send_message(
        callback_query.from_user.id,
        "Полный доступ доступен в платной версии."
        "\nВ платной версии:"
        "❇️25 медитаций"
...
@dp.callback_query_handler(
    lambda c: c.data and c.data.startswith("Main_menu"), state="*"
)
async def main_menu_callback(callback_query: types.CallbackQuery, state: FSMContext):
    await FSM_classes.HabitSleep.none.set()
    await FSM_classes.HabitRemind.none.set()
    await FSM_classes.MultiDialog.menu.set()
    await bot.send_message(
        callback_query.from_user.id,
        "Вы в главном меню. Не знаете что делать дальше?"
...
@dp.message_handler(
    state=(
        FSM_classes.MultiDialog.test_weariness
        or FSM_classes.MultiDialog.test_control
        or FSM_classes.MultiDialog.test_selfefficacy
        or FSM_classes.MultiDialog.test_typeperson
        or FSM_classes.MultiDialog.test_stress
    )
)
...
@dp.message_handler(state=FSM_classes.HabitRemind.choose_action)
async def reply_habit_remind(message: types.Message, state: FSMContext):
    await Body.choose_actionhab(message, state)
    await log_users(message)


@dp.message_handler(state=FSM_classes.HabitRemind.choose_day)
async def reply_habit_remind(message: types.Message, state: FSMContext):
    await Body.habbit_choose_day(message, state)
    await log_users(message)


@dp.message_handler(state=FSM_classes.HabitRemind.choose_time)
async def reply_habit_remind(message: types.Message, state: FSMContext):
    await Body.habbit_choose_time(message, state)
    await log_users(message)


@dp.message_handler(state=FSM_classes.HabitRemind.choose_data)
async def reply_habit_remind(message: types.Message, state: FSMContext):
    await Body.data_remind(message, state)
    await log_users(message)
...
@dp.message_handler(state=FSM_classes.HabitRemind.data_text)
async def get_text(message: types.Message, state: FSMContext):
    text = message.text
    db_remind = sqlite3.connect("Databases/Current_habits.db")
    cur_remind = db_remind.cursor()
    cur_remind.execute(
        "UPDATE remind SET remind_data = ? WHERE user_id = ?",
        (text, message.from_user.id),
    )
    db_remind.commit()
    await bot.send_message(message.from_user.id, "Напоминание успешно создано!")


@dp.message_handler(content_types=["voice"], state=FSM_classes.HabitRemind.data_audio)
...
@dp.message_handler(content_types=["photo"], state=FSM_classes.HabitRemind.data_photo)
...
@dp.message_handler(content_types=["video"], state=FSM_classes.HabitRemind.data_video)
async def get_photo(message: types.Message, state: FSMContext):
...
@dp.callback_query_handler(
    text_startswith="day_", state=FSM_classes.HabitRemind.choose_day
)
async def day(call: types.CallbackQuery, state: FSMContext):
    db_remind = sqlite3.connect("Databases/Current_habits.db")
    cur_remind = db_remind.cursor()
    # cur_remind.execute(
    #    "UPDATE remind SET day = NULL WHERE user_id = ?", (
    #        call.from_user.id,))
    # db_remind.commit()
    call_data = call.data
    day_choose = call_data.split("_")[1]
    # days = cur_remind.execute(
    #    "SELECT day FROM remind WHERE user_id = ?", (call.from_user.id,)
    # ).fetchone()[0]
    # days = [*days]
    # days += day_choose
    # print(days)
    # cur_remind = db_remind.cursor()
    # cur_remind.execute(
    #    "UPDATE remind SET day = ? WHERE user_id = ?",
    #    (
    #        str(days),
    #        call.from_user.id,
    #    ),
    ...
    # db_remind.commit()
    if day_choose == "done":
        await bot.send_message(call.from_user.id, "Готово!")
        await FSM_classes.HabitRemind.choose_time.set()
        await bot.send_message(call.from_user.id, "Введите время в формате ЧЧ:ММ")
    db_remind.close()


async def scheduler_habbit_remind():
    db_remind = sqlite3.connect("Databases/Current_habits.db")
    cur_remind = db_remind.cursor()
    nowTime = datetime.datetime.utcnow() + timedelta(hours=3, minutes=00)
    nowDay = str(
        (datetime.datetime.utcnow() + timedelta(hours=3, minutes=00)).isocalendar()[2]
    )
    remind_time_first = cur_remind.execute(
        "SELECT user_id, remind_data, type FROM remind WHERE time = ?",
        (nowTime.strftime("%H:%M"),),
    ).fetchall()
    for user_remind in range(len(remind_time_first)):
        day = list(
            cur_remind.execute(
                "SELECT day FROM remind WHERE user_id = ?",
                (remind_time_first[user_remind][0],),
            ).fetchone()
        )
        if str(nowDay) in list(day)[0]:
            try:
                if remind_time_first[user_remind][2] == "photo":
                    await bot.send_photo(
                        chat_id=remind_time_first[user_remind][0],
                        photo=remind_time_first[user_remind][1],
                    )
                elif remind_time_first[user_remind][2] == "audio":
                    await bot.send_voice(
                        chat_id=remind_time_first[user_remind][0],
                        voice=remind_time_first[user_remind][1],
                    )
                elif remind_time_first[user_remind][2] == "text":
                    await bot.send_message(
                        chat_id=remind_time_first[user_remind][0],
                        text=remind_time_first[user_remind][1],
                    )
                elif remind_time_first[user_remind][2] == "video":
                    await bot.send_video(
                        chat_id=remind_time_first[user_remind][0],
                        video=remind_time_first[user_remind][1],
                    )
                await asyncio.sleep(0.1)
            except BotBlocked:
                cur_remind.execute(
                    "UPDATE remind SET user_id = 0 WHERE user_id = ?",
                    (remind_time[user_remind][0],),
                )
                db_remind.commit()
                cur_remind.execute("DELETE FROM remind WHERE user_id = ?", (int(0),))
                db_remind.commit()

    remind_time = cur_remind.execute(
        "SELECT user_id, remind_data, type FROM remind WHERE timeRemind = ?",
        (nowTime.strftime("%H:%M"),),
    ).fetchall()

    for user_remind in range(len(remind_time)):
        day = list(
            cur_remind.execute(
                "SELECT day FROM remind WHERE user_id = ?",
                (remind_time[user_remind][0],),
        ...
        if str(nowDay) in list(day)[0]:
            try:
                if remind_time[user_remind][2] == "photo":
                    await bot.send_photo(
                        chat_id=remind_time[user_remind][0],
                        photo=remind_time[user_remind][1],
                        reply_markup=Markups.answer_to_remind,
                    )
                elif remind_time[user_remind][2] == "audio":
                    await bot.send_voice(
                        chat_id=remind_time[user_remind][0],
                        voice=remind_time[user_remind][1],
                        reply_markup=Markups.answer_to_remind,
                    )
                elif remind_time[user_remind][2] == "text":
                    await bot.send_message(
                        chat_id=remind_time[user_remind][0],
                        text=remind_time[user_remind][1],
                        reply_markup=Markups.answer_to_remind,
                    )
                elif remind_time[user_remind][2] == "video":
                    await bot.send_video(
                        chat_id=remind_time[user_remind][0],
                        video=remind_time[user_remind][1],
                        reply_markup=Markups.answer_to_remind,
                    )
                await asyncio.sleep(0.1)
            except BotBlocked:
...
async def get_answer(call: types.CallbackQuery):
    db_remind = sqlite3.connect("Databases/Current_habits.db")
    remind_answer = db_remind.cursor()
    nowTime = str(
        remind_answer.execute(
            "SELECT timeRemind FROM remind WHERE user_id = ?", (call.from_user.id,)
        ).fetchone()
    )

    call_data = call.data
    nowDay = datetime.datetime.utcnow() + timedelta(hours=3, minutes=00)
    answer = call_data.split("_")[1]
    db_remind.commit()
    if answer == "+":
        print("Сделал")
        remind_answer.execute(
            "INSERT INTO answer_remind VALUES(?, ?, ?, ?, ?)",
            (call.from_user.id, call.from_user.username, nowDay, nowTime, answer),
        )
        db_remind.commit()
    elif answer == "-":
        print("not Сделал")
...
async def scheduler_sleep_message_wakeup():
    ...
    now = datetime.datetime.utcnow() + timedelta(hours=3, minutes=0)
    users_wakeup = cur_scheduler.execute(
        "SELECT user_id FROM sleep WHERE wakeup = ?", (now.strftime("%H:%M"),)
    ).fetchall()
    for user_wakeup in range(len(users_wakeup)):
        try:
            await bot.send_message(
                chat_id=users_wakeup[user_wakeup][0],
                text="Пора вставать! "
            ...
            cur_scheduler.execute(
                "UPDATE sleep SET user_id = 0 WHERE user_id = ?",
                (users_wakeup[user_wakeup][0],),
            )
...
async def scheduler_sleep_message_bedtime():
    ...
    users_bedtime = cur_scheduler.execute(
        "SELECT user_id FROM sleep WHERE bedtime = ?", (now.strftime("%H:%M"),)
    ).fetchall()
    for user_bedtime in range(len(users_bedtime)):
        try:
            await bot.send_message(
                chat_id=users_bedtime[user_bedtime][0],
...
async def scheduler_sleep():
    ...
    schedule.every(1).minute.do(scheduler_habbit_remind)
    while True:
