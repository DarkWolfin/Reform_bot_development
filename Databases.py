...
async def db_start():
    ...
    cur_data.execute("CREATE TABLE IF NOT EXISTS profile(user_id INT PRIMARY KEY, first_name TEXT, username TEXT, active TEXT)")
    db_data.commit()
    cur_data.execute("CREATE TABLE IF NOT EXISTS affirmation(user_id INT PRIMARY KEY, first_name TEXT, username TEXT)")
    db_data.commit()
    ...
    cur_test_weariness.execute("CREATE TABLE IF NOT EXISTS points(user_id INT PRIMARY KEY, username TEXT, count INT, points INT)")
    db_test_weariness.commit()
    ...
    cur_test_selfefficacy.execute("CREATE TABLE IF NOT EXISTS points(user_id INT PRIMARY KEY, username TEXT, count INT, points INT)")
    ...
    cur_test_control.execute("CREATE TABLE IF NOT EXISTS points(user_id INT PRIMARY KEY, username TEXT, count INT, points INT)")
    ...
    cur_habit_sleep.execute("CREATE TABLE IF NOT EXISTS sleep(user_id INT PRIMARY KEY, username TEXT, active TEXT, bedtime TEXT, wakeup TEXT)")
    db_habit_sleep.commit()

    db_course_anxiety = sq.connect('Databases/Courses.db')
    cur_course_anxiety = db_course_anxiety.cursor()
    cur_course_anxiety.execute("CREATE TABLE IF NOT EXISTS anxiety(user_id INT PRIMARY KEY, username TEXT, interested TEXT)")
    db_course_anxiety.commit()


async def data_profile(user_id, first_name, username):
    user = cur_data.execute("SELECT 1 FROM profile WHERE user_id == '{key}'".format(key=user_id)).fetchone()
    if not user:
        cur_data.execute("INSERT INTO profile VALUES(?, ?, ?, ?)", (user_id, first_name, username, 'Активен'))
        db_data.commit()

async def affirmation(user_id, first_name, username):
    user = cur_data.execute("SELECT 1 FROM affirmation WHERE user_id == '{key}'".format(key=user_id)).fetchone()
    if not user:
        cur_data.execute("INSERT INTO affirmation VALUES(?, ?, ?)", (user_id, first_name, username))
        db_data.commit()

# PSY tests
async def pre_points_test_weariness(user_id, username):
    user = cur_test_weariness.execute("SELECT 1 FROM points WHERE user_id == '{key}'".format(key=user_id)).fetchone()
    if not user:
...
async def pre_points_test_selfefficacy(user_id, username):
    user = cur_test_selfefficacy.execute("SELECT 1 FROM points WHERE user_id == '{key}'".format(key=user_id)).fetchone()
    if not user:
...
async def pre_points_test_control(user_id, username):
    user = cur_test_control.execute("SELECT 1 FROM points WHERE user_id == '{key}'".format(key=user_id)).fetchone()
    if not user:
...
async def pre_points_test_typeperson(user_id, username):
    user = cur_test_typeperson.execute("SELECT 1 FROM points WHERE user_id == '{key}'".format(key=user_id)).fetchone()
    if not user:
...
async def prehabit_sleep_db(user_id, username):
    user = cur_habit_sleep.execute("SELECT 1 FROM sleep WHERE user_id == '{key}'".format(key=user_id)).fetchone()
    if not user:
...
async def course_anxiety_db(user_id, username, interested):
    user = cur_course_anxiety.execute("SELECT 1 FROM anxiety WHERE user_id == '{key}'".format(key=user_id)).fetchone()
    if not user:
