...
async def answer_selfefficacy(callback_query: types.CallbackQuery):
    ...
    three = int(3)
    four = int(4)
    cur_selfefficacy.execute("UPDATE points SET count = (count + ?) WHERE user_id = ?",
                             (one, callback_query.from_user.id))
    if callback_query.data[-1] == '1':
        cur_selfefficacy.execute("UPDATE points SET points = points + ? WHERE user_id = ?",
                                 (one, callback_query.from_user.id))
    elif callback_query.data[-1] == '2':
        cur_selfefficacy.execute("UPDATE points SET points = points + ? WHERE user_id = ?",
                                 (two, callback_query.from_user.id))
    elif callback_query.data[-1] == '3':
        cur_selfefficacy.execute("UPDATE points SET points = points + ? WHERE user_id = ?",
                                 (three, callback_query.from_user.id))
    elif callback_query.data[-1] == '4':
        cur_selfefficacy.execute("UPDATE points SET points = points + ? WHERE user_id = ?",
                                 (four, callback_query.from_user.id))
    db_selfefficacy.commit()
    if int(cur_selfefficacy.execute('SELECT count FROM points WHERE user_id = ?',
                                    (callback_query.from_user.id,)).fetchone()[0]) != 10:
        current_question_index = cur_selfefficacy.execute('SELECT count FROM points WHERE user_id = ?',
                                                          (callback_query.from_user.id,)).fetchone()[0]
        await bot.edit_message_text(chat_id=callback_query.from_user.id,
                                    text=selfefficacy_questions[int(current_question_index)],
                                    message_id=callback_query.message.message_id,
                                    reply_markup=selfefficacy_answer)
    else:
        await bot.delete_message(callback_query.from_user.id, callback_query.message.message_id)
        if int(cur_selfefficacy.execute('SELECT points FROM points WHERE user_id = ?',
                                        (callback_query.from_user.id,)).fetchone()[0]) < 27:
            ...
            await save_user_action(user_id=callback_query.from_user.id, action='Прошёл тест Psy_selfefficacy (Низкая самоэффективность)')
                ...
                cur_selfefficacy.execute('SELECT points FROM points WHERE user_id = ?',
                                         (callback_query.from_user.id,)).fetchone()[0]) < 36):
            await bot.send_message(callback_query.from_user.id,
            ...
            await bot.send_message(callback_query.from_user.id,
                                   ...
                                   'вы добьетесь всего, чего пожелаете!'
...
def register_handlers_Psy_selfefficacy(dp: Dispatcher):
    dp.register_callback_query_handler(answer_selfefficacy,
                                       text=['selfefficacy_answer_1', 'selfefficacy_answer_2', 'selfefficacy_answer_3',
                                             'selfefficacy_answer_4'])
